        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            {experiences.map((exp, index) => (
              <RevealOnScroll key={exp.title} delay={100 + index * 120}>
                <Card
                  className="bg-[#EDE8F5] dark:bg-[#181B23] border-[#ADBBD4] dark:border-[#3D52A0] shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-xl font-semibold text-[#3D52A0] dark:text-[#7091E6] mb-2">
                          {exp.title}
                        </CardTitle>
                        <p className="text-lg font-medium text-[#8697C4] dark:text-[#ADBBD4] mb-1">
                          {exp.company}
                        </p>
                  ...
                  <CardContent>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {exp.technologies.map((tech, techIndex) => (
                        <span
                          key={techIndex}
                          className="px-3 py-1 bg-white dark:bg-[#23263a] text-[#3D52A0] dark:text-[#7091E6] rounded-full text-sm font-medium border border-[#ADBBD4] dark:border-[#3D52A0]"
                        >
                          {tech}
                        </span>
                      ))}
